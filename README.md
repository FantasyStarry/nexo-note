...
A local markdown-based notes CLI for your knowledge base. 一个本地 Markdown 笔记知识库 CLI 工具，支持分类、标签、搜索、版本管理，可被 agent 通过 skill 调用。
...
```powershell
# Windows
.\skills\nexo-note\install.ps1

# macOS / Linux
./skills/nexo-note/install.sh
```

会将 skill 文件复制到 `~/.trae-cn/skills/nexo-note/`（Trae）或 `~/.claude/skills/nexo-note/`（Claude Code）。
